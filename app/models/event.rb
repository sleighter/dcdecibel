class Event < ActiveRecord::Base
  ...
  belongs_to :venue
  has_many :tickets
  scope :upcoming, where("event_datetime > ?",Time.now - 5.hours)
  scope :just_announced, where("created_at > ? AND event_datetime > ?",7.days.ago.to_datetime,Time.now - 5.hours)
  scope :tonight, where("event_datetime > ?",Time.now - 5.hours)
